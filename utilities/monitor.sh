...
mem_status() {
  ...
  case "${pressure_level}" in
    ...
    *) printf "Unknown (%s)" "${pressure_level}" ;;
  esac
}

thermal_status() {
  pressure_level=$(sudo powermetrics -i 500 -n 1 -s thermal 2>/dev/null | awk -F ': *' '/Current pressure level/ {print $2; exit}')
  case "${pressure_level}" in
...
disk_usage() {
  ...
  local usage_percentage=$(( (used_blocks * 100) / total_blocks ))
  ...
  printf "${bold}${color}%s%%${normal}" "${usage_percentage}"
}
